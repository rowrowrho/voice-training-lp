...
(function (global) {
  ...
  const CONFIG = {
    ...
    // と判断して乗り換えるか。
    // 値を大きくするほど「一度ロックした音程にしがみつく」動きになる。
    // 実声では第2倍音が基音より一時的に強く出続けることがあり、乗り換えが
    // 早すぎると正しい音から誤った倍音側へ乗っ取られてしまうため、
    // ここでは実質無効化に近い大きな値にしている。
    // (最初の認識が合っているケースでは、こちらの方が安定する)
    OCTAVE_RELOCK_STREAK_FRAMES: 100000,

    // CLEAR!になってから次のTARGET(半音上)へ自動で進むまでの待機時間(ms)
